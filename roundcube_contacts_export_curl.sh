...
curl -o contacts.vcf -ssl -L -b cookies -c cookies -X GET -F"_task=addressbook" -F"_action=export" -F"_source=0" -F"_token=$token" -s "$url/?_task=addressbook&_source=0&_action=export" > /dev/null
